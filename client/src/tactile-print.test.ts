...
describe("tactile print visual system", () => {
  it("defines grain, halftone, overprint, edge-boil, and motion safeguards", () => {
  ...
  it("applies tactile layers to the requested home, menu, review, and scoop surfaces", () => {
    expect(home).toContain("ice-cream-destination print-surface print-surface--dark print-halftone");
    expect(menu).toContain('className="menu-page print-paper"');
    expect(menu).toContain("menu-chapter-break print-surface print-halftone");
    expect(menu).toContain("menu-dish-card print-edge-boil");
    expect(reviews).toContain("reviews-section section-pad print-paper");
    expect(reviews).toContain("review-card review-card--proof print-edge-boil");
  });
});
